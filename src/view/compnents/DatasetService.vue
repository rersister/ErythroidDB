    ...
    <div>
        ...
        <div class="card">
			...
			<Row v-if=ifshowvis>
                        ...
                        <Col span="20"><a :href="getVisHref()">{{visualize_site}}</a></Col>                         
            </Row>
			...
			<div v-if=ifshowAnalContent>
				...
				<Row style="background:#eee;padding:20px" :gutter="4">
					...
					<Col span="4" v-for="(AnalyzeType,index) in AnalyzeTypeList" :key='index' :value="AnalyzeType.name" >
							...
							<div class='anal_div' >
								<router-link :to="AnalyzeType.link">
								
										<!-- <img width="20%" height="10%" src="@/assets/img/red_sys.jpg"> -->
										<h3 class='h3_title'>{{ AnalyzeType.name }}</h3>
...
export default {
	...
	methods:{
		...
		getDatasetSequenceType(){
            ...
            getDatasetSequenceType(this.$store.state.app.CurrentPageToken).then(res => {                              
					...
					if ( data.sequencing_type.indexOf("Single Cell") > -1  ){
						...
						if(data.visualize_site != null){
							...
							this.AnalyzeTypeList = [
									{
										name:"Single Cell t_SNE & Feature",
										link:'PlotTSNE'
									},
				...
				// 	if((experiment_type_list.length) > 1 ){
				// 		_this.showSubAnalContent = true
				// 		_this.SubAnalContent = 'See Subseries'
...
<style>
	...
	.anal_div{
		...
		text-align:center;
		overflow: hidden;
		padding:calc((15/1920) * 100vw) calc((5/1920) * 100vw) calc((15/1920) * 100vw) calc((5/1920) * 100vw);
		background: white;
		border: 1px;
		border-color: black;
		border-radius: 4px;
